...
git checkout s_mt/scripts.sql
echo "--changeset builder:update_$GIT_HASH_MIN runOnChange:true dbms:postgresql" >> ./s_mt/scripts.sql
echo "update s_mt.t_sys_setting set cv_value='$GIT_HASH' where ck_id='core_db_commit';" >> ./s_mt/scripts.sql
echo "update s_mt.t_sys_setting set cv_value='$GIT_VERSION' where ck_id='core_db_major_version';" >> ./s_mt/scripts.sql
echo "update s_mt.t_sys_setting set cv_value=to_char(CURRENT_TIMESTAMP, 'dd.MM.YYYY HH24:mm:ss') where ck_id='core_db_deployment_date';" >> ./s_mt/scripts.sql
