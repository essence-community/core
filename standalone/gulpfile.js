...
gulp.task('create_os_package', async () => {
  ...
  let versionstr = fs.readFileSync(path.join(__dirname, '..', 'backend', 'dbms', 's_mt', 'version.sql'), { encoding: 'utf-8' }).toString()
  ...
  versionstr += "update s_mt.t_sys_setting set cv_value=to_char(CURRENT_TIMESTAMP, 'dd.MM.YYYY HH24:mm:ss') where ck_id='core_db_deployment_date';\n"
  fs.writeFileSync(path.join(__dirname, '..', 'backend', 'dbms', 's_mt', 'version.sql'), versionstr, {
    encoding: 'utf-8'
  })
  const coreZip = new AdmZip()
  coreZip.addLocalFolder(path.join(__dirname, '..', 'frontend', 'packages', '@essence', 'essence-constructor-website', 'build'))
